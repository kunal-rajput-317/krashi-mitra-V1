...
def snapshot(monkeypatch):
    ...
    def use(fetched_on, impressions=1_531_851):
        payload = {
            "fetched_on": fetched_on, "window_days": 28,
            "start": "2026-08-19", "end": "2026-09-15",
            "totals": {"impressions": impressions, "clicks": 13944,
                       "ctr": 0.91, "urls": 14189, "per_day": 54709},
            "series": [], "sections": {"/bhav": {"i": 964798, "c": 9421, "urls": 13064}},
            "devices": [{"k": "MOBILE", "pct": 93.4}],
            "countries": [{"k": "ind", "pct": 99.4}],
        }
...
def test_public_page_never_prints_a_figure_even_with_a_fresh_snapshot(client, snapshot):
    ...
    for figure in ("15,31,851", "1531851", "13,944", "13944",
                   "14,189", "14189", "54,709", "54709", "9,64,798", "964798"):
...
def test_payment_is_recorded_by_a_human_not_inferred(client, clean_sponsors):
    ...
    client.post("/admin/sponsor/acme-seeds/payment", json={}, auth=AUTH)
    listed = client.get("/admin/sponsor", auth=AUTH).json()["sponsors"]
