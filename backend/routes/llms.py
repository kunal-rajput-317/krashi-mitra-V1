...
_CORE = [
    ...
    ("/pashupalan/anda-rate", "अंडे का रेट",     "Daily egg rate (anda rate) for ~34 Indian cities and production/consumption zones, sourced from the National Egg Co-ordination Committee (NECC) and refreshed every morning. Quoted both ways the trade uses — rupees per egg and rupees per 100 eggs — with the previous day's change, a 30-day trend and a same-week-last-year comparison per zone. The sheet distinguishes NECC SUGGESTED prices from PREVAILING (actually trading) prices and so do these pages; NECC states its suggested prices are advisory and not mandatory, and that clarification is reproduced on every page. Not a farm-gate price and not an offer from us. Full URL list in /pashupalan/sitemap.xml."),
    ("/pashupalan",          "पशुपालन",        "Livestock hub: the daily egg rate above, plus Hindi guides to dairy/milk production, goat rearing, fish farming, beekeeping and lumpy skin disease. Only poultry carries a daily price, because only the egg rate is published nationally each day."),
...
def _build() -> str:
    ...
    lines += [
        ...
        *_top_crop_lines(),
        f"- [Egg rate hub]({SITE}/pashupalan/anda-rate): today's NECC egg rate in every listed city, and a page per zone",
        f"- [Full price-page URL list]({SITE}/bhav/sitemap.xml)",
        f"- [Product page URL list]({SITE}/product/sitemap.xml)",
        f"- [Egg rate URL list]({SITE}/pashupalan/sitemap.xml)",
        "",
        "## लेख (farming guides)",
        "",
    ]
