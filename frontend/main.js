async function resetChat() {
    ...
    await fetch(`${window.KRASHIMITRA_API_BASE || location.origin}/reset?crop=${crop}`, { method: "POST" });
...
async function askQuestion() {
  ...
  try {
    const res = await fetch(`${window.KRASHIMITRA_API_BASE || location.origin}/ask`, {
      method: "POST",
